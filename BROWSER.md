...
This document covers the command reference and internals of jstack's headless browser.
...
jstack's browser is a compiled CLI binary that talks to a persistent local Chromium daemon over HTTP. The CLI is a thin client — it reads a state file, sends a command, and prints the response to stdout. The server does the real work via [Playwright](https://playwright.dev/).
...
Each server session generates a random UUID as a bearer token. The token is written to the state file (`.jstack/browse.json`) with chmod 600. Every HTTP request must include `Authorization: Bearer <token>`. This prevents other processes on the machine from controlling the browser.
...
4. **Navigate to the extension folder:** Press **Cmd+Shift+G** in the file picker to open "Go to folder", then paste one of these paths:
   - Global install: `~/.claude/skills/jstack/extension`
   - Dev/source: `<jstack-repo>/extension`
...
5. **Pin it:** Click the puzzle piece icon (Extensions) in the toolbar → pin "jstack browse"
6. **Set the port:** Click the jstack icon → enter the port from `$B status` or `.jstack/browse.json`
7. **Open Side Panel:** Click the jstack icon → "Open Side Panel"
...
| **Connection pill** | Small "jstack" pill in the bottom-right corner of every page when connected |
...
Each workspace gets its own isolated browser instance with its own Chromium process, tabs, cookies, and logs. State is stored in `.jstack/` inside the project root (detected via `git rev-parse --show-toplevel`).

| Workspace | State file | Port |
|-----------|------------|------|
| `/code/project-a` | `/code/project-a/.jstack/browse.json` | random (10000-60000) |
| `/code/project-b` | `/code/project-b/.jstack/browse.json` | random (10000-60000) |
...
| `BROWSE_STATE_FILE` | `.jstack/browse.json` | Path to state file (CLI passes to server) |
...
| **jstack browse** | **~3s** | **~100-200ms** | **0 tokens** (plain text stdout) |

The context overhead difference compounds fast. In a 20-command browser session, MCP tools burn 30,000-40,000 tokens on protocol framing alone. jstack burns zero.
...
| File | Role |
|------|------|
| `browse/src/cli.ts` | Entry point. Reads `.jstack/browse.json`, sends HTTP to the server, prints response. |
...
The active skill lives at `~/.claude/skills/jstack/`. After making changes:

1. Push your branch
2. Pull in the skill directory: `cd ~/.claude/skills/jstack && git pull`
3. Rebuild: `cd ~/.claude/skills/jstack && bun run build`

Or copy the binary directly: `cp browse/dist/browse ~/.claude/skills/jstack/browse/dist/browse`
