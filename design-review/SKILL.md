...
description: |
  ...
  wants to polish the look of a live site. (jstack)
allowed-tools:
  - Bash
  - Read
  - Write
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"design-review","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.jstack/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
~/.claude/skills/jstack/bin/jstack-timeline-log '{"skill":"design-review","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
...
```bash
~/.claude/skills/jstack/bin/jstack-question-log '{"skill":"design-review","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
If `NEEDS_SETUP`:
1. Tell the user: "jstack browse needs a one-time build (~10 seconds). OK to proceed?" Then STOP and wait.
...
**Find the jstack designer (optional — enables target mockup generation):**
...
```bash
eval "$(~/.claude/skills/jstack/bin/jstack-slug 2>/dev/null)"
REPORT_DIR="$HOME/.jstack/projects/$SLUG/designs/design-audit-$(date +%Y%m%d)"
...
**Local:** `.jstack/design-reports/design-audit-{domain}-{YYYY-MM-DD}.md`

**Project-scoped:**
```bash
eval "$(~/.claude/skills/jstack/bin/jstack-slug 2>/dev/null)" && mkdir -p ~/.jstack/projects/$SLUG
```
Write to: `~/.jstack/projects/{slug}/{user}-{branch}-design-audit-{datetime}.md`
...
```
~/.jstack/projects/$SLUG/designs/design-audit-{YYYYMMDD}/
...
If the jstack designer is available and the finding involves visual layout, hierarchy, or spacing (not just a CSS value fix like wrong color or font-size), generate a target mockup showing what the corrected version should look like:
...
eval "$(~/.claude/skills/jstack/bin/jstack-slug 2>/dev/null)" && mkdir -p ~/.jstack/projects/$SLUG
```
Write a one-line summary to `~/.jstack/projects/{slug}/{user}-{branch}-design-audit-{datetime}.md` with a pointer to the full report in `$REPORT_DIR`.
...
```bash
~/.claude/skills/jstack/bin/jstack-learnings-log '{"skill":"design-review","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
