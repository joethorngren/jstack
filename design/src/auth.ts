 ...
 *
 * Resolution order:
 * 1. ~/.jstack/openai.json → { "api_key": "sk-..." }
...
const CONFIG_PATH = path.join(process.env.HOME || "~", ".jstack", "openai.json");

export function resolveApiKey(): string | null {
  // 1. Check ~/.jstack/openai.json
  try {
...
/**
 * Save an API key to ~/.jstack/openai.json with 0600 permissions.
 */
...
export function requireApiKey(): string {
  ...
  if (!key) {
    ...
    console.error("  or save to ~/.jstack/openai.json: { \"api_key\": \"sk-...\" }");
