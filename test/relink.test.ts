...
// Every test in this file shells out to jstack-config + jstack-relink (bash scripts
...
describe('jstack-relink (#578)', () => {
  ...
  // e.g., `qa -> jstack/qa` gets discovered as "jstack-qa", not "qa".
  ...
  // FIRST INSTALL: --no-prefix must create ONLY flat names, zero jstack-* pollution
  test('first install --no-prefix: only flat names exist, zero jstack-* entries', () => {
    ...
    // Expected: qa, ship, review, plan-ceo-review, jstack-upgrade (its real name)
    expect(entries.sort()).toEqual(['jstack-upgrade', 'plan-ceo-review', 'qa', 'review', 'ship']);
    // No jstack-qa, jstack-ship, jstack-review, jstack-plan-ceo-review
    const leaked = entries.filter(e => e.startsWith('jstack-') && e !== 'jstack-upgrade');
  ...
  test('first install --prefix: only jstack-* entries exist, zero flat names', () => {
    ...
    expect(entries.sort()).toEqual([
      'jstack-plan-ceo-review', 'jstack-qa', 'jstack-review', 'jstack-ship', 'jstack-upgrade',
    ]);
    // No unprefixed qa, ship, review, plan-ceo-review
    const leaked = entries.filter(e => !e.startsWith('jstack-'));
    expect(leaked).toEqual([]);
  });
  ...
  test('switching no-prefix to prefix removes all flat entries completely', () => {
    setupMockInstall(['qa', 'ship', 'review', 'jstack-upgrade']);
...
describe('upgrade migrations', () => {
  const MIGRATIONS_DIR = path.join(ROOT, 'jstack-upgrade', 'migrations');
  ...
  test('v0.15.2.0 migration runs jstack-relink', () => {
    const content = fs.readFileSync(path.join(MIGRATIONS_DIR, 'v0.15.2.0.sh'), 'utf-8');
    expect(content).toContain('jstack-relink');
  });
