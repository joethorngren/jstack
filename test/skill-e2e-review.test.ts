...
describeIfSelected('Review Dashboard Via Attribution', ['review-dashboard-via'], () => {
  ...
  beforeAll(() => {
    ...
    // jstack-review-read reads from ~/.jstack/projects/$SLUG/$BRANCH-reviews.jsonl
    // For the test, we'll write a mock jstack-review-read script that returns our test data
    ...
    // Write a mock jstack-review-read that returns our test data
    const mockBinDir = path.join(dashDir, '.mock-bin');
    fs.mkdirSync(mockBinDir, { recursive: true });
    fs.writeFileSync(path.join(mockBinDir, 'jstack-review-read'), [
      '#!/usr/bin/env bash',
    ...
    fs.chmodSync(path.join(mockBinDir, 'jstack-review-read'), 0o755);
...
Instead of running ~/.claude/skills/jstack/bin/jstack-review-read, run this mock: ${mockBinDir}/jstack-review-read
