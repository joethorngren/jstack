...
describeIfSelected('Session Intelligence E2E', [
  ...
  beforeAll(() => {
    ...
    for (const script of [
      'jstack-timeline-log', 'jstack-timeline-read', 'jstack-slug',
      'jstack-learnings-log', 'jstack-learnings-search',
    ]) {
    ...
    // Compute slug (same logic as jstack-slug without git remote)
  ...
  // Write a timeline event via jstack-timeline-log, read it back via jstack-timeline-read.
  ...
  testConcurrentIfSelected('timeline-event-flow', async () => {
    ...
    const logBin = path.join(workDir, 'bin', 'jstack-timeline-log');
    const readBin = path.join(workDir, 'bin', 'jstack-timeline-read');
    const env = { ...process.env, JSTACK_HOME: jstackHome };
    ...
    // Verify jstack-timeline-read output includes the events
