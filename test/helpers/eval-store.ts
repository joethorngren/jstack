 ...
 * ~/.jstack/projects/$SLUG/evals/{version}-{branch}-{tier}-{timestamp}.json,
...
const LEGACY_EVAL_DIR = path.join(os.homedir(), '.jstack-dev', 'evals');

/**
 * Detect project-scoped eval dir via jstack-slug.
 * Falls back to legacy ~/.jstack-dev/evals/ if slug detection fails.
 */
export function getProjectEvalDir(): string {
  try {
    // Try repo-local jstack-slug first, then global install
    const localSlug = spawnSync('bash', ['-c', '.claude/skills/jstack/bin/jstack-slug 2>/dev/null || ~/.claude/skills/jstack/bin/jstack-slug 2>/dev/null'], {
    ...
    if (output) {
      ...
      if (slugMatch && slugMatch[1]) {
        const dir = path.join(os.homedir(), '.jstack', 'projects', slugMatch[1], 'evals');
