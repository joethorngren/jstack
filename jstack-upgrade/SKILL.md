...
```bash
cd "$_ROOT"
git rm -r --cached .claude/skills/jstack/ 2>/dev/null || true
...
```bash
MIGRATIONS_DIR="$INSTALL_DIR/jstack-upgrade/migrations"
...
Migrations are idempotent bash scripts in `jstack-upgrade/migrations/`. Each is named
