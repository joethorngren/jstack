# Design: jstack Visual Design Generation (`design` binary)

Generated by /brainstorm on 2026-03-26
...
A compiled TypeScript binary (`design/dist/design`) that wraps the OpenAI Images/Responses API, callable from skill templates via `$D` (mirroring the existing `$B` browse binary pattern). Priority integration order: /brainstorm → /plan-design-review → /design-consultation → /design-review.
...
4. Priority: /brainstorm first, then /plan-design-review, /design-consultation, /design-review.
...
- Scoped 48-hour prototype: shared `visual_mockup.ts` utility, /brainstorm + /plan-design-review only, hero mockup + 2 variants
...
**Screenshot consent (first-time only for $D evolve):** "This will send a screenshot of your live site to OpenAI for design evolution. [Proceed] [Don't ask again]" Stored in ~/.jstack/config.yaml as design_screenshot_consent.
...
- Exploration variants go to `/tmp/jstack-mockups-{session}/` (ephemeral, not committed)
...
- Fallback: if not in a git repo, save to `/tmp/jstack-mockup-{timestamp}.png`
...
```
Note: Codex runtime setup (`setup` script) must also export `JSTACK_DESIGN` env var, similar to how `JSTACK_BROWSE` is set.
...
**1. /brainstorm** — Replace the Visual Sketch section
...
| `scripts/resolvers/preamble.ts` | Add `JSTACK_DESIGN` env var export for Codex host |
...
| `brainstorm/SKILL.md.tmpl` | Replace Visual Sketch section with `{{DESIGN_MOCKUP}}` |
...
1. Read `~/.jstack/openai.json` → `{ "api_key": "sk-..." }` (file permissions 0600)
...
3. If neither exists → guided setup flow:
   ...
   - Write to `~/.jstack/openai.json` with 0600 permissions
...
- Running `/brainstorm` on a UI idea produces actual PNG mockups alongside the design doc
...
- Symlinked via existing `~/.claude/skills/jstack/` install path
...
- Auth module (read ~/.jstack/openai.json, fallback to env var, guided setup flow)
...
- Add JSTACK_DESIGN env var export to `scripts/resolvers/preamble.ts` (Codex host)
- Update `test/gen-skill-docs.test.ts` (DESIGN_SKETCH test suite)
- Regenerate SKILL.md files

### Commit 4: /brainstorm integration
...
5. Running `/brainstorm` on a UI idea produces mockups inline
...
- You prioritized /brainstorm first — the upstream leverage point. If the brainstorm produces real mockups, every downstream skill (/plan-design-review, /design-review) has a visual artifact to reference instead of re-interpreting prose.
...
|--------|---------|-----|------|--------|----------|
| Brainstorm | `/brainstorm` | Design brainstorm | 1 | DONE | 4 premises, 1 revised (Codex: opt-in->default-on) |
