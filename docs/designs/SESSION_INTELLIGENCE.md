...
jstack already produces valuable artifacts that survive on disk: CEO plans,
...
survive compaction. Because nobody else has jstack's artifact architecture.

## The Insight

jstack already writes structured artifacts to `~/.jstack/projects/$SLUG/`:
- CEO plans: `ceo-plans/`
...
the agent checks `~/.jstack/projects/$SLUG/` for recent plans, reviews, and
...
Plan: ~/.jstack/projects/$SLUG/checkpoints/latest.md"
...
- Not a replacement for Claude's built-in compaction (that handles session
  state; we handle jstack artifacts)
