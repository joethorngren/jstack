# Design: JStack Self-Learning Infrastructure

Generated by /brainstorm + /plan-ceo-review + /plan-eng-review on 2026-03-28
...
JStack runs 30+ skills across sessions but learns nothing between them. A /review
...
Structured, typed, confidence-scored learnings that every jstack skill can read and
write. The goal: after 20 sessions on the same codebase, jstack knows every
...
YC founders building with AI. The people who run jstack on real codebases 20+ times
...
| **JStack** | **Per-project JSONL** | **Cross-session, cross-skill** | **Typed, scored, decaying** |

---

## State Systems

jstack has four distinct persistence layers. They share storage patterns
(JSONL in `~/.jstack/projects/$SLUG/`) but serve different purposes:
...
### Release 1: "JStack Learns" (v0.13-0.14) — SHIPPED

**Headline:** Every session makes the next one smarter.

What shipped:
- Learnings persistence at `~/.jstack/projects/{slug}/learnings.jsonl`
...
- Integration into /review, /ship, /plan-*, /brainstorm, /investigate, /retro
...
- E1: Adaptive specialist gating, auto-skip specialists with 0-finding track record.
  Store per-project hit rates via jstack-learnings-log. User can force with --security etc.
...
- E5: Cross-review finding dedup, read jstack-review-read for prior review entries.
  Suppress findings matching a prior user-skipped finding.
- E7: Specialist performance tracking, log per-specialist metrics via jstack-review-log.
...
- Session timeline: every skill auto-logs start/complete events to
  `~/.jstack/projects/$SLUG/timeline.jsonl`. Local-only, never sent anywhere,
...
- Predictive skill suggestion: if your last 3 sessions follow a pattern
  (review, ship, review), jstack suggests what you probably want next.
...
- Timeline binaries: `bin/jstack-timeline-log` and `bin/jstack-timeline-read`.
...
**Headline:** JStack respects your time without compromising your safety.
...
The self-learning roadmap was inspired by ideas from the [Compound Engineering](https://github.com/nicobailon/compound-engineering) project by Nico Bailon. Their exploration of learnings persistence, parallel review agents, and autonomous pipelines catalyzed the design of JStack's approach. We adapted every concept to fit JStack's template system, voice, and architecture rather than porting directly.
