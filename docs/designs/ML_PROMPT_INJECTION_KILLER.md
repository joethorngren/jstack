...
**Date:** 2026-03-28
**CEO Plan:** ~/.jstack/projects/garrytan-jstack/ceo-plans/2026-03-28-sidebar-prompt-injection-defense.md

## The Problem

The jstack Chrome extension sidebar gives Claude bash access to control the browser.
...
```typescript
// Public API -- any jstack component can call these
...
```json
// ~/.jstack/security/attempts.jsonl
{
...
```
AskUserQuestion:
  "jstack just blocked a prompt injection attempt from {domain}. These detections
   are rare and valuable for improving defenses for all jstack users. Can we
...
2. Cache to ~/.jstack/models/browsesafe-bench/ (not re-downloaded in CI)
...
- jstack becomes the only open source tool with native-speed prompt injection detection.
...
**Limitation:** macOS-only (Linux would need OpenBLAS FFI). But jstack already
...
- [ ] Implement `loadModel()` with download-on-first-use to ~/.jstack/models/
