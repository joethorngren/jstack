...
description: |
  ...
  test a site, verify a deployment, dogfood a user flow, or file a bug with screenshots. (jstack)
allowed-tools:
  - Bash
  - Read
  - AskUserQuestion
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"jstack","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.jstack/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
~/.claude/skills/jstack/bin/jstack-timeline-log '{"skill":"jstack","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
...
If `PROACTIVE` is `false`: do NOT proactively invoke or suggest other jstack skills during
this session. Only run skills the user explicitly invokes. This preference persists across
sessions via `jstack-config`.
...
- User describes a new idea, asks "is this worth building", wants to brainstorm → invoke `/brainstorm`
...
- User asks to upgrade jstack → invoke `/jstack-upgrade`
...
If the user opts out of suggestions, run `jstack-config set proactive false`.
If they opt back in, run `jstack-config set proactive true`.

# jstack browse: QA Testing & Dogfooding
...
`$B` is the browse binary (resolved from `$_ROOT/.claude/skills/jstack/browse/dist/browse` or `~/.claude/skills/jstack/browse/dist/browse`).
