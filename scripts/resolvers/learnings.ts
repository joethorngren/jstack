 ...
 *
 * Learnings are stored per-project at ~/.jstack/projects/{slug}/learnings.jsonl.
 ...
 * by jstack-learnings-search ("latest winner" per key+type).
 *
 * Cross-project discovery is opt-in. The resolver asks the user once via
 * AskUserQuestion and persists the preference via jstack-config.
 */
...
\`\`\`bash
_CROSS_PROJ=$(${ctx.paths.binDir}/jstack-config get cross_project_learnings 2>/dev/null || echo "unset")
echo "CROSS_PROJECT: $_CROSS_PROJ"
if [ "$_CROSS_PROJ" = "true" ]; then
  ${ctx.paths.binDir}/jstack-learnings-search --limit 10 --cross-project 2>/dev/null || true
else
  ${ctx.paths.binDir}/jstack-learnings-search --limit 10 2>/dev/null || true
fi
\`\`\`
...
If A: run \`${ctx.paths.binDir}/jstack-config set cross_project_learnings true\`
If B: run \`${ctx.paths.binDir}/jstack-config set cross_project_learnings false\`
...
\`\`\`bash
${binDir}/jstack-learnings-log '{"skill":"${ctx.skillName}","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
\`\`\`
