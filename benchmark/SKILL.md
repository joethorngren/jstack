...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"benchmark","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.jstack/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
~/.claude/skills/jstack/bin/jstack-timeline-log '{"skill":"benchmark","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
...
mkdir -p .jstack/benchmark-reports
mkdir -p .jstack/benchmark-reports/baselines
```
...
Write to `.jstack/benchmark-reports/baselines/baseline.json`.
...
Write to `.jstack/benchmark-reports/{date}-benchmark.md` and `.jstack/benchmark-reports/{date}-benchmark.json`.
